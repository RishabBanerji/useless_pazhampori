...
judgeButton.addEventListener("click", () => {
    ...
    let idx = 0;
    let ti = setInterval(() => {
        judgeThinking.textContent = sequences[idx % sequences.length];
        idx++;
    }, 600);
    setTimeout(() => {
        clearInterval(ti);
...
timerStart.addEventListener("click", () => {
    ...
    timerId = setInterval(() => {
        ...
        if (rem <= 0) {
            clearTimer();
            timerDisplay.textContent = "Done! (maybe)";
            timerFinished = true;
            return;
        }
...
timerStop.addEventListener("click", () => {
    clearTimer();
    timerDisplay.textContent = timerFinished ? "Already Done!" : "Stopped. Time is an illusion.";
});

timerChaos.addEventListener("click", () => {
    timerMins.value = Math.floor(Math.random() * 20) + 1;
    timerStart.click();
});
...
function makePrediction() {
    ...
    setTimeout(() => {
        predText.textContent = "🤔 calculating";
...
function bananaBubble() {
    ...
    for (let i = 0; i < 6; i++) {
        setTimeout(() => {
            let last = stockHistory[stockHistory.length - 1];
            let newC = Math.max(1, last.c * crashFactor);
            stockHistory.push({
                o: last.c,
                c: newC,
                h: newC + Math.random() * 2,
                l: newC - Math.random() * 2
            });
            if (stockHistory.length > 120) stockHistory.shift();
            drawChart();
        }, i * 200);
    ...
    setTimeout(() => {
        stockNote.textContent = "Bubble crashed. Market full of crumbs.";
    }, 1600);
}
...
stockCandleBtn.addEventListener("click", () => {
...
refreshStatus.onclick = () => {
    deliveryStatus.textContent = rand(statuses);
};
...
weirdBtn.onclick = () => {
    deliveryStatus.textContent = rand(statuses).toUpperCase() + " !!!";
};